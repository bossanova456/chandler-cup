...
import MatchUpCardListRowHeader from "../components/MatchUpCardListRowHeader";
...
describe('MatchUpCardList', () => {
  ...
  it('Should render list of match up cards', () => {
    const matchUpCards = wrapper.find(MatchUpCard);
    expect(wrapper).toMatchSnapshot();
    expect(matchUpCards.length).toBe(matchUpData.matchUps.length);
    expect(matchUpCards.length >= playerData.weeklyPicks[weekNum].matchUps.length);
  })

  it('Should render a single match up header card', () => {
    const headerCard = wrapper.find(MatchUpCardListRowHeader);
